<x-app-layout>
    <div class="bg-white p-6 rounded-xl shadow-sm card">
        <div class="flex flex-col md:flex-row justify-between items-start md:items-center mb-6">
            <h3 class="text-lg font-semibold text-gray-800">Tambah Item Masuk</h3>
        ...
        <form action="{{ route('dashboard.itemsIn.update', $itemsIn) }}" method="POST">
            @csrf
            @method('PUT')
            <div class="mx-auto grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                    ...
                    <select name="item_id" id="item_id" class="w-full mt-1 border-gray-300 rounded-lg">
                        @foreach ($items as $id => $name)
                            <option value="{{ $id }}"
                                {{ old('item_id', $itemsIn->item_id) == $id ? 'selected' : '' }}>
                                {{ $name }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <x-text-input id="quantity" name="quantity" type="number" min="1"
                        value="{{ old('quantity', $itemsIn->quantity) }}" required />
                    <x-input-error :messages="$errors->get('quantity')" class="mt-1" />
                </div>

                {{-- Tanggal Masuk --}}
                <div>
                    <x-input-label for="date_in" value="Tanggal Masuk" />
                    <x-text-input id="date_in" name="date_in" type="date"
                        value="{{ old('date_in', $itemsIn->date_in) }}" required />
                    <x-input-error :messages="$errors->get('date_in')" class="mt-1" />
                </div>
            </div>
